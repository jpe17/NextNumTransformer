...
class VisionTransformer(nn.Module):
    def __init__(self, patch_dim=36, embed_dim=16, num_patches=100, 
        ...
        self.encoder_pos_embedding = nn.Parameter(_build_sincos_pos_embed(num_patches, embed_dim), requires_grad=False)
        ...
        self.decoder_pos_embedding = nn.Parameter(torch.randn(1, max_seq_len, embed_dim))
    ...
    def forward(self, src_patches, tgt_tokens):
        ...
        x = x + self.encoder_pos_embedding.to(x.device)
        ...
        y = y + self.decoder_pos_embedding[:, :seq_len, :]
        ...
        for i in range(self.num_layers):
            ...
            encoder_output_norm = self.encoder_norm2_layers[i](encoder_output) # Normalize encoder output
